...
def check_reference_versions(document, lock):
    ...
    for node in document.iter():
        ...
        if tag == 'PlaceholderReference':
            ...
            resolution = next((child.text or '' for child in node
...
def check(root=ROOT, release=False, report=None, build_evidence=None, library=None):
            ...
            if tag == 'POU':
                ...
                for child in node:
                    if child.tag == 'LineIds':
                        ...
                        errors.append(f'{path}: code after LineIds metadata is ignored by XAE')
            ...
            if tag == 'POU':
                if 'TcForge/Modules' in path.as_posix():
                    ...
                    if re.search(r'\bAT\s+%[IQ]', declaration, re.I):
                        errors.append(f'{path}: library block allocates application hardware')
                    for method in node.findall('Method'):
                        decl = method.findtext('Declaration', '')
                        body = method.findtext('Implementation/ST', '')
                        name = method.get('Name', '')
                        commands = {'Ack', 'Reset', 'Home', 'Start', 'Stop', 'Abort',
                                    'Pause', 'Proceed', 'Retry', 'Advance', 'Retract',
                                    'SetOn', 'SetOff', 'SetValue', 'ForceSafe'}
                        if name in commands and re.search(r'\beRequester\s*:', decl):
                            statements = re.sub(r'//[^\n]*|\(\*.*?\*\)', '', body, flags=re.S).strip()
                            if not statements.startswith(name + ' := F_ValidateRequester('):
                                errors.append(f'{path}: {name} must validate identity before command logic')
                        if 'TcRpcEnable' in decl:
                            if not method.get('Name', '').startswith('Operator') or 'eRequester :' in decl:
                                errors.append(f'{path}: RPC must use a fixed-identity Operator wrapper')
                            body = method.findtext('Implementation/ST', '')
                            expected = '_operatorMailbox.GetResult(' if method.get('Name') == 'OperatorCommandResult' else '_operatorMailbox.Submit('
                            if expected not in body or 'requestId : ULINT' not in decl or 'THIS^.' in body:
                                errors.append(f'{path}: RPC must use the bounded mailbox with a caller request ID')
                        if method.get('Name') == '_DispatchOperatorCommand':
                            if 'E_Requester.OPERATOR' not in method.findtext('Implementation/ST', ''):
    ...
    for path, document in documents.items():
        if path.suffix.lower() == '.plcproj':
            errors.extend(f'{path}: {message}' for message in check_reference_versions(document, lock))
    ...
    if release:
        ...
        reports = [] if report is None else ([report] if isinstance(report, (str, Path)) else list(report))
        if build_evidence is None or library is None:
            errors.append('Release blocked: provide --build-evidence and --library with fresh bound build/runtime results')
        else:
            try:
        ...
        for path in (root / 'TwinCAT').rglob('*.plcproj'):
            if re.search(r'<DefaultResolution>[^<]*\*', path.read_text(encoding='utf-8')):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test-report', type=Path, action='append', help='Repeat for bound 1 ms and 10 ms reports')
    parser.add_argument('--build-evidence', type=Path)
    parser.add_argument('--library', type=Path)
    args = parser.parse_args()
    errors, files, suites, tests = check(release=args.release, report=args.test_report,
                                           build_evidence=args.build_evidence, library=args.library)
    for error in errors:
        print(error, file=sys.stderr)
    print(f'{files} XML files; {suites} suites; {tests} test declarations. Source checks only.')
