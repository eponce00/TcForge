...
class OnlineChangeEvidenceTests(unittest.TestCase):
    def setUp(self):
        self.before = dict(boot=123, advance=1, retract=0, faulted=0, inhibited=0)
        self.after = dict(self.before, boot=124, session=0, advance=0, inhibited=1)
    ...
    def test_download_restart_or_no_change_cannot_pass(self):
        for count, boot in ((5, 124), (0, 124), (7, 124), (6, 123), (6, 0)):
            with self.subTest(count=count, boot=boot):
                with self.assertRaises(AssertionError):
                    validate_change(self.before, dict(self.after, boot=boot), 5, count, 'moving')

    def test_unhealthy_or_absent_motion_precondition_rejected(self):
        for field, value in (('advance', 0), ('retract', 1), ('faulted', 1)):
            with self.assertRaises(AssertionError):
                validate_change(dict(self.before, **{field: value}), self.after, 5, 6, 'moving')

    def test_either_coil_or_uninhibited_intent_rejected(self):
        for field, value in (('advance', 1), ('retract', 1), ('inhibited', 0), ('session', 12)):
            with self.assertRaises(AssertionError):
                validate_change(self.before, dict(self.after, **{field: value}), 5, 6, 'moving')

    def test_lifecycle_history_must_continue_without_reinitialization(self):
        before = dict(applicationMarker=49374, markerAtInitialization=0, completedSequence=2,
                      diagnosticLastFault=1, alarmActive=True, alarmLatched=True, alarmAcked=False,
                      cycles=100)
        after = dict(before, cycles=200)
        validate_retained(before, after)
        for field, value in (('applicationMarker', 0), ('completedSequence', 0), ('cycles', 1),
                             ('diagnosticLastFault', 0), ('alarmLatched', False)):
            with self.assertRaises(AssertionError):
    ...
    def test_compatible_edit_requires_continuous_motion_and_same_session(self):
        ...
        for field, value in (('boot', 124), ('session', 0), ('advance', 0),
                             ('retract', 1), ('faulted', 1), ('inhibited', 1)):
            with self.subTest(field=field), self.assertRaises(AssertionError):
                validate_change(before, dict(before, **{field: value}), 5, 6, 'moving', 'preserve')
        with self.assertRaises(AssertionError):
