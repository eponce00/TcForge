...
class DevelopmentLayoutTests(unittest.TestCase):
    ...
    def test_installed_fallback_is_rejected(self):
        ...
        self.assertTrue(any('reference TcForge source' in e for e in check_development_layout(self.root)))

    def test_support_library_must_be_referenceable(self):
        self.edit('TwinCAT/TcForgeReference.plcproj', '<VirtualLibrary>true</VirtualLibrary>', '<VirtualLibrary>false</VirtualLibrary>')
        self.assertTrue(any('enable referenced-library' in e for e in check_development_layout(self.root)))

    def test_task_identity_collision_is_rejected(self):
        self.edit('TwinCAT/TcForge.tsproj', 'AmsPort="353"', 'AmsPort="350"')
        self.assertTrue(any('duplicate system task AmsPort' in e for e in check_development_layout(self.root)))

    def test_source_library_boot_build_is_rejected(self):
        self.edit('TwinCAT/TcForge.sln',
        ...
        self.assertTrue(any('must not build boot projects' in e for e in check_development_layout(self.root)))

    def test_missing_source_library_configuration_is_rejected(self):
        self.edit('TwinCAT/TcForge.Example.sln',
                  '{C50D021A-2A7E-435E-BD9B-20A91E08C1DA}.Release|TwinCAT RT (x64).ActiveCfg',
                  '{00000000-0000-0000-0000-000000000000}.Release|TwinCAT RT (x64).ActiveCfg')
        self.assertTrue(any('missing explicit solution configuration' in e for e in check_development_layout(self.root)))

    def test_missing_library_source_is_rejected(self):
        self.edit('TwinCAT/TcForge.Tests.tsproj', 'Name="TcForge"', 'Name="Unexpected"')
        self.assertTrue(any('membership' in e for e in check_development_layout(self.root)))
