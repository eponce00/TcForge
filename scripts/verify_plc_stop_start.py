...
def exercise(args, pyads, hold, evidence, trace):
    ...
    with pyads.Connection(args.target, args.port) as plc:
        ...
        try:
            ...
            session = AssemblySession(rpc, trace, TwoPositionCylinder(0.5))
            ...
            session.until(lambda s: s['advance'] == 1 and 0.1 < session.plant.position < 0.8, command=2)
            session.plant.jammed = True
            before = session.tick()
            require(before['advance'] == 1 and not before['faulted'], 'No healthy advance before STOP')
            evidence['before'] = before
            old_boot, old_session, old_frame = session.boot, session.session, session.sequence
            plant = session.plant
            plc.write_control(pyads.ADSSTATE_RESET if args.operation == 'ads-reset' else pyads.ADSSTATE_STOP, 0, 0, pyads.PLCTYPE_BYTE)
            require(plc.read_state()[0] == pyads.ADSSTATE_STOP, 'STOP not confirmed')
            evidence['stopped_coils'] = [plc.read_by_name('MAIN.simulation.outAdvance', pyads.PLCTYPE_BOOL),
            ...
            after = rpc.snapshot()
            while not (after['boot'] != old_boot and after['owner'] > 0 and after['cycle'] > 0) and time.perf_counter() < deadline:
                time.sleep(0.002)
                after = rpc.snapshot()
            ...
            require(after['boot'] != old_boot and after['session'] == 0,
            ...
            evidence['stale_exchange'] = rpc.call('exchange', old_session, old_boot, old_frame + 1,
                                                 before['cycle'], 0, 0, 1, 1, 2)
            require(evidence['stale_claim'] == 34 and evidence['stale_exchange'] == 34,
                    'Stale operation was not rejected')
            session = None
            plant.jammed = False
            session = AssemblySession(rpc, trace, plant)
            require(not session.tick()['advance'], 'Claim resumed old motion')
            session.until(lambda s: s['state'] == 0, command=4)
            session.tick()
            ...
            require(plant.position == 0, 'Recovery cycle did not return home')
            ...
            try:
                if rpc:
                    ...
                    require(not evidence['cleanup']['advance'] and not evidence['cleanup']['retract'],
...
def main():
    ...
    parser.add_argument('--operation', choices=('stop-start', 'ads-reset'), default='stop-start')
    ...
    if args.lifecycle_state and args.operation != 'ads-reset':
        parser.error('--lifecycle-state requires --operation ads-reset; STOP/RUN preserves volatile state')
    ...
    for hold in ((0.03, 0.5) if args.operation == 'stop-start' else (0.03,)):
        evidence = {'target': args.target, 'port': args.port, 'hold_seconds': hold, 'operation': args.operation, 'passed': False}
        case = ET.SubElement(suite, 'testcase', name=f'{args.operation}DuringAdvance_{hold}s')
        start = time.perf_counter()
        try:
            with (args.output / f'trace-{hold}.jsonl').open('w', encoding='utf-8') as trace:
    ...
    (args.output / 'evidence.json').write_text(json.dumps(results, indent=2), encoding='utf-8')
