...
def run(args, evidence, trace):
    ...
    paths = {
        name: control / name
        for name in ("ready", "trigger", "cancel", "started", "result")
    }
    settings = dict(
        ...
        onlineSettings=str(control / "online-settings.xml"),
    ...
    try:
        ...
        while not paths["ready"].exists():
            require(
                process.poll() is None, "XAE warmup failed; inspect restart-helper.log"
            )
        ...
        session = AssemblySession(rpc, trace, AssemblyPlant(travel_s=0.5))
        ...
        before = session.tick()
        require(
            before["clamp_advance"] == 1
            and not before["clamp_retract"]
            and not before["faulted"],
            "Restart precondition is not healthy active advance",
        )
        require(0 < session.plant.clamp.position < 1, "Plant is not jammed mid-travel")
        evidence["before"] = before
        evidence["position_at_restart"] = session.plant.clamp.position
        ...
        evidence["last_exchange_to_detected_loss_s"] = (
            time.perf_counter() - last_success_time
        )
        evidence["frame_before_loss"] = session.sequence
        require(
            paths["started"].exists(),
            "Endpoint failed before the restart API was invoked",
        )
        rpc.close()
        rpc = None
        ...
        require(
            ...
            "Old boot requests were not rejected",
        )
        ...
        require(
            ...
            "Stale requests mutated new runtime",
        )
        ...
        session.tick()
        require(
            session.tick(5)["response"] == 0, "Post-restart explicit Reset rejected"
        )
        session.tick()
        ...
        require(
            plant.clamp.position == 0.0,
            "Explicit recovered cycle did not retract the clamp",
        )
        ...
        if session and rpc:
            try:
                ...
                require(
                    not evidence["released"]["clamp_advance"]
                    and not evidence["released"]["clamp_retract"],
        ...
        if process and process.poll() is None:
            try:
                ...
                cleanup_errors.append(
                    "Helper cleanup exceeded bound; its process tree was terminated: "
                    + killed.stdout
                    + killed.stderr
                )
        if process:
        ...
        if paths["result"].exists() and "restart_result" not in evidence:
            try:
                evidence["restart_result"] = json.loads(
...
def main():
    ...
    parser.add_argument(
        "--lifecycle-state",
        choices=("seed", "safe", "reset"),
        help="Also qualify actual fixture persistence",
    )
    parser.add_argument(
        "--operation",
        choices=("system-restart", "cold-reset"),
        default="system-restart",
    )
    parser.add_argument("--target", required=True, help="Dedicated runtime AMS Net ID")
    parser.add_argument(
        "--port", required=True, type=int, help="Deployed simulation PLC ADS port"
    )
    parser.add_argument("--solution", required=True, type=Path)
    parser.add_argument(
        "--automation", required=True, type=Path, help="Existing TcAutomation.exe"
    )
    parser.add_argument(
        "--transport", required=True, type=Path, help="Existing SimulationRpc.exe"
    )
    ...
    evidence = {
        "scope": args.operation
        + " during simulated advance; persistence="
        + str(args.lifecycle_state)
        + "; excludes power-loss durability",
        "target": args.target,
        "port": args.port,
        "solution": str(args.solution),
    ...
    case = ET.SubElement(
        suite,
        "testcase",
        name=args.operation + "DuringAdvance",
        classname="TcForge.Simulation",
    )
    ...
    try:
        ...
        ET.ElementTree(suite).write(
            args.output / "restart-junit.xml", encoding="utf-8", xml_declaration=True
        )
    print(
        "PASS" if evidence["passed"] else "FAIL",
        args.operation + "DuringAdvance",
        flush=True,
    )
