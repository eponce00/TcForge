...
'@
 ...
 function FindPou($node) {
  ...
  if ([IO.Path]::GetFileNameWithoutExtension($node.Name) -eq 'FB_ReferenceMachine') {return ,$node}
 ...
 @{Command=$chosen;Available=$true} | ConvertTo-Json | Set-Content -LiteralPath $cfg.inventory -Encoding UTF8
...
def validate_change(before, after, count_before, count_after, mode, policy='recover'):
    ...
    if policy == 'preserve':
        require(before['boot'] > 0 and after['boot'] == before['boot'], 'Compatible edit changed session epoch')
        require(before['session'] > 0 and after['session'] == before['session'], 'Compatible edit lost session')
        require(not after['faulted'] and after['inhibited'] == before['inhibited'], 'Compatible edit interrupted operation')
        if mode == 'moving':
            for snapshot in (before, after):
                require(snapshot['advance'] and not snapshot['retract'] and not snapshot['faulted'],
    ...
    require(before['boot'] > 0 and after['boot'] > 0 and after['boot'] != before['boot'],
...
def validate_retained(before, after):
    for field in ('applicationMarker', 'markerAtInitialization', 'completedSequence',
                  'diagnosticLastFault', 'alarmActive', 'alarmLatched', 'alarmAcked'):
        require(after[field] == before[field], field + ' changed across online change')
    require(after['cycles'] > before['cycles'], 'Runtime cycle history did not continue')


def environment():
    env = os.environ.copy()
    common = Path(os.environ.get('ProgramFiles(x86)', r'C:\Program Files (x86)')) / 'Beckhoff' / 'TwinCAT'
    env['PATH'] = str(common / 'Common64') + ';' + str(common / 'Common32') + ';' + env.get('PATH', '')
    return env
...
def activate(args, path):
    ...
    with path.open('w', encoding='utf-8') as log:
        process = subprocess.Popen(['powershell.exe', '-NoProfile', '-NonInteractive', '-File', str(script),
...
def run(args, evidence):
    ...
    source = root / 'TwinCAT/TcForgeExample/Reference/FB_ReferenceMachine.TcPOU'
    ...
    try:
        ...
        prepared = True
        with (control / 'helper.log').open('w', encoding='utf-8') as log, (control / 'trace.jsonl').open('w', encoding='utf-8') as trace:
            ...
            while not paths['ready'].exists():
                require(process.poll() is None, 'Online-change preparation failed; inspect helper.log')
                require(time.perf_counter() < deadline, 'Online-change preparation timed out')
                time.sleep(.1)
            ...
            evidence['observations'] = observations
            require(after is not None, 'OnlineChangeCnt never incremented; command or confirmation did not apply')
            ...
            if args.policy == 'preserve':
                ...
                plant.jammed = False
                if args.mode == 'moving':
                    session.until(lambda s: s['retract'])
                    evidence['continued_cycle'] = session.until(lambda s: s['state'] == 16)
                deadline = time.perf_counter() + args.timeout
                ...
                require(paths['started'].exists() and result.get('Dispatched') is True,
            ...
            stale_exchange = rpc.call('exchange', old_session, before['boot'], old_frame + 1,
                                      before['cycle'], 0, 0, 1, 1, 2)
            evidence['stale_requests'] = dict(claim=stale_claim, exchange=stale_exchange)
            require(stale_claim == 34 and stale_exchange == 34, 'Old-epoch requests were not rejected')
            unchanged = rpc.snapshot()
            require(unchanged['session'] == 0 and unchanged['applied'] == after['applied'] and
                    not unchanged['advance'] and not unchanged['retract'], 'Stale requests mutated new epoch')
            session = None
            require(paths['started'].exists(), 'Counter changed before requested online command')
            # No new simulator session exists yet; old commands cannot be replayed.
            deadline = time.perf_counter() + args.timeout
            while process.poll() is None:
                require(time.perf_counter() < deadline, 'Online helper did not finish')
                time.sleep(.01)
            require(process.returncode == 0, 'Online-change helper did not finish successfully')
            result = json.loads(paths['result'].read_text(encoding='utf-8-sig'))
            evidence['command_result'] = result
            ...
            require(not reconnected['advance'] and not reconnected['retract'], 'New connection resumed old motion')
            session.until(lambda s: s['state'] == 0, command=4)
            session.tick()
            ...
            session.until(lambda s: s['advance'], command=2)
            session.until(lambda s: s['retract'])
...
def main():
    ...
    parser.add_argument('--platform', required=True, choices=('TwinCAT RT (x64)', 'TwinCAT OS (x64)'))
    parser.add_argument('--kind', required=True, choices=('implementation', 'declaration'))
    ...
    (args.output / 'online-change-evidence.json').write_text(json.dumps(evidence, indent=2), encoding='utf-8')
    ET.ElementTree(suite).write(args.output / 'online-change-junit.xml', encoding='utf-8', xml_declaration=True)
