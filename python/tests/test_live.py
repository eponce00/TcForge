...
class FakeRpc:
    def __init__(self):
        ...
        self.exchanges = 0
    def snapshot(self):
        return dict(boot=self.boot, session=self.session, applied=self.applied,
                    cycle=self.applied + 1, advance=0, retract=0)
...
class LiveContractTests(unittest.TestCase):
    ...
    def test_uncertain_outcome_is_never_replayed(self):
        ...
        self.assertEqual([event['event'] for event in events], ['exchange_attempt', 'session_failed'])
    ...
    def test_scheduling_gap_fails_before_exchange(self):
        rpc = FakeRpc()
        client = AssemblySession(rpc, io.StringIO())
        with patch('tcforge_sim.live.time.perf_counter', return_value=client.last_time + .21):
            with self.assertRaises(TimeoutError):
                client.tick()
        self.assertEqual(rpc.exchanges, 0)
    ...
    def test_admitted_frame_records_applied_feedback(self):
        ...
        self.assertEqual(events, ['exchange_attempt', 'exchange_admitted', 'exchange_applied'])
